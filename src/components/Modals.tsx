...
function Modals({ currentTable, setVisibleColums }: FilterProps) {
  ...
  return (
    <div>
      ...
      <div>
        {users.length > 0 && (
          <button onClick={createModal.handleOpenModal}>create column</button>
        )}
        {users.length > 0 && (
          <button onClick={deleteModal.handleOpenModal}>delete column</button>
        )}
        {users.length > 0 && (
          <button onClick={filterModal.handleOpenModal}>filter column</button>
        )}
        {users.length > 0 && (
